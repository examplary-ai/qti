...
describe("QtiElement", () => {
  ...
  describe("registerNamespace", () => {
    test("it registers a custom namespace", () => {
      const item = new QtiItem({ identifier: "test-item" });
      item.registerNamespace("examplary", "http://examplary.ai/xsd/examplary_v1p0");

      const xml = item.buildXml();

      expect(xml).toContain('xmlns:examplary="http://examplary.ai/xsd/examplary_v1p0"');
    });
  ...
  describe("addNamespacedElement", () => {
    test("it adds element with string content", () => {
      const item = new QtiItem({ identifier: "test-item" });
      item.registerNamespace("examplary", "http://examplary.ai/xsd/examplary_v1p0");
      item.addNamespacedElement("examplary", "scoring-criteria", "rubric-based");

      const xml = item.buildXml();

      expect(xml).toContain("<examplary:scoring-criteria>rubric-based</examplary:scoring-criteria>");
    });
    ...
    test("it adds element with object content", () => {
      ...
      expect(xml).toContain("<examplary:description>Excellent work</examplary:description>");
    });
    ...
    test("it adds multiple elements", () => {
      ...
      expect(xml).toContain("<examplary:scoring-criteria>rubric-based</examplary:scoring-criteria>");
      expect(xml).toContain("<examplary:difficulty>medium</examplary:difficulty>");
